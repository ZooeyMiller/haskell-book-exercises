...
mostPopularElement def str = fst $ foldr
  (\t@(_, amt) t2@(_, amt2) -> if amt > amt2 then t else t2)
  (def, 0)
  chars
  where chars = nub $ map (\char -> (char, getTotalInstances str char)) str
...
costOfMostPopularLetter phone string = totalTaps
 where
  ...
  totalTaps =
    foldr (\(_, x) y -> x + y) 0 $ concat $ map (reverseTaps phone) $ replicate
      totalInstance
      mostPop
...
coolestWord xs =
  mostPopularElement "" $ map (map toLower) $ concat $ map words xs
